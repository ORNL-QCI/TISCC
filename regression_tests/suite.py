...
slicer_locations = list(filter(lambda location: os.path.exists(f"{location}/tiscc"), POSSIBLE_SLICER_LOCATIONS))
if not slicer_locations:
    print("No slicer found", file=sys.stderr)
    exit(1)
slicer_location = slicer_locations[0]

print(f"Using tiscc executable from {slicer_location}")
